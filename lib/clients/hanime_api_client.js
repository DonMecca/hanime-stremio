...
class HanimeApiClient {
  ...
  async getVideoData(slug, maxRetries = 2) {
    ...
    // hanime.tv/api/v8/video is a retired endpoint (404 SPA). It is kept as a
    // last-resort probe, but it MUST be bounded: this call previously had no
    // timeout at all, and from Netlify's egress it stalled ~8s per request for
    // any slug missing from the index (measured 2026-09-22), which is exactly the
    // "click a video, wait 20s" symptom. A 3s ceiling keeps that path cheap.
    ...
    try {
      return await this._retryRequest(
        async () => {
          ...
          logger.debug('Hanime video API: no data (retired endpoint)', {
            slug,
        ...
        // One attempt is enough for a retired endpoint; retrying multiplies the
        // stall without ever succeeding.
        0
      );
    } catch (error) {
      return null;
  ...
  async _getSearchIndex(allowRefresh = true) {
    ...
    this._searchIndexPromise = (async () => {
      ...
      // Current shape: { data: [...], ads: {...} }
      if (!Array.isArray(hits) && Array.isArray(hits?.data)) {
        hits = hits.data;
      }
