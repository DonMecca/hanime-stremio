...
// How long a cold-instance pool probe may delay the first handshake. Only enough
// to catch a host that answers immediately; the rotation below is what actually
// finds a live proxy (measured: the winning host answered at ~3.5s, past this
// budget, and the rotation still recovered). Anything slower than this runs on in
// the background and benefits the next request.
const COLD_START_PROBE_BUDGET_MS = 1200;

// Per-attempt handshake timeouts. A live proxy answers in ~1.5-2s, so 4s is
// generous; the point of the cap is that a dead pool fails in seconds instead of
// stalling the reader. `direct` is longer because a direct attempt is not time-boxed
// by the rotation budget.
const PROXY_ATTEMPT_TIMEOUT_MS = 4000;
const DIRECT_ATTEMPT_TIMEOUT_MS = 15000;

// A live handshake needs roughly this long. The rotation stops rather than start an
// attempt it cannot finish inside the overall budget.
...
class HanimeWebApiClient {
  ...
  async _postHandshake(slug, headers, proxyUrl = null, timeoutMs = null) {
    ...
    // Per-attempt timeout. A working proxy completes the handshake in ~1.5-2s, so
    // 4s is generous for a live host and keeps a dead pool from turning into a long
    // stall. The rotation passes a smaller value for the final attempt so the
    // attempt cannot overshoot the overall budget.
    const attemptTimeoutMs = timeoutMs || (proxyUrl ? PROXY_ATTEMPT_TIMEOUT_MS : DIRECT_ATTEMPT_TIMEOUT_MS);
  ...
  async getStreamsForSlug(slug) {
    ...
    // - Fresh instance (never probed): give a probe a SHORT budget, then proceed
    //   regardless. A cold pass that has to fetch and test a public list costs
    //   ~19s measured 2026-09-22, and blocking the request on it is exactly the
    //   "20 seconds before anything loads" symptom. Past the budget the pass
    //   keeps running in the background and the request rotates instead.
    ...
    if (this.proxyPool.isEnabled()) {
      try {
        ...
        const refreshed = await this.proxyPool.refreshIfDegraded({
          force: coldStart,
          waitMs: coldStart ? COLD_START_PROBE_BUDGET_MS : 0
        });
        if (refreshed) {
          ...
          logger.info('Hanime handshake: cold-start probe exceeded budget, rotating now', {
            slug,
            budgetMs: COLD_START_PROBE_BUDGET_MS
          });
        }
      } catch (error) {
    ...
    // 9s covers a verified-live host (finishes in ~1.5-2s) plus two dead-host
    // timeouts, and bounds the failure case at roughly 9s + the cold-start probe
    // rather than the 20s a reader was seeing.
    const PROXY_BUDGET_MS = Number(process.env.HANDSHAKE_PROXY_BUDGET_MS) || 9000;
    ...
    for (const attempt of attempts) {
      ...
      const remaining = PROXY_BUDGET_MS - elapsed;

      // Stop rather than start an attempt that cannot finish in budget. Checking
      // only BETWEEN attempts let the last one pay a full connect timeout past the
      // budget, which is how a 15s budget produced a 20s response.
      if (attempted > 0 && remaining < MIN_ATTEMPT_MS) {
        errors.push(`budget exhausted after ${attempted} attempt(s) in ${elapsed}ms`);
      ...
      attempted += 1;
      try {
        const response = await this._postHandshake(
          slug,
          headers,
          attempt.proxyUrl,
          attempt.proxyUrl ? Math.min(PROXY_ATTEMPT_TIMEOUT_MS, remaining) : null
        );
        ...
        // of each paying a full timeout. Bounded: on a warm pool past the throttle
        // this pass can run for ~15s, which would dwarf the rotation it is meant to
        // help. Past the budget it keeps going in the background instead.
        if (!receivedHttpResponse && !probeTriggered) {
          probeTriggered = true;
          try {
            const refreshed = await this.proxyPool.refreshIfDegraded({
              force: true,
              waitMs: MIN_ATTEMPT_MS
            });
            if (refreshed) {
