...
// Seed order matters: a cold instance has no probe results, so it walks this list
// in order and every dead host that hangs costs a full attempt timeout. Ranked by a
// live probe on 2026-09-22: one host answered the handshake (1.4s), three refused
// instantly (cheap to try), four hung until the timeout (expensive — kept last so
// the rotation budget stops before reaching them).
//
// This list is only a starting point. The pool re-probes, re-orders and replenishes
// itself at runtime, so a roster that has rotted does not need a code change.
const DEFAULT_PROXIES = [
  'http://37.59.125.131:8888', // alive, ~1.4s
  'http://146.103.43.35:3128', // fast refuse
  'http://181.39.25.196:8118', // fast refuse
  'http://109.120.184.202:1080', // fast refuse
...
class HandshakeProxyPool {
  ...
  constructor(options = {}) {
    ...
    // The configured/curated hosts are the known-good baseline. They must never
    // be dropped from the pool by replenishment: one flaky probe sample used to
    // be enough to evict them, which threw away working proxies (including the
    // host that had just served streams) and replaced them with public-list
    // junk. Replenishment may only ADD alongside them.
    this.seedProxies = this.proxies.slice();
  ...
  /**
   * Probe one proxy by requesting the exact host the handshake needs.
   *
   * A reachable proxy returns 401 from the bodyless handshake, so an HTTP
   * response normally proves the tunnel works. 407 is the exception: that comes
   * from the proxy itself demanding credentials, so the request never reached the
   * origin. Counting those as alive filled the pool with unusable hosts that then
   * failed every handshake with 407 (measured 2026-09-22: 6 of 8 "alive" hosts).
   */
  async probeProxy(proxyUrl, timeoutMs = PROBE_TIMEOUT_MS) {
    ...
    try {
      ...
      return { proxyUrl, alive: true, status, ms: Date.now() - startedAt };
    } catch (error) {
      return {
        proxyUrl,
        alive: false,
  ...
  async replenishFromSource() {
    ...
    // Keep every curated host, plus any non-curated host we have not written off.
    // Verified additions are preferred over unverified ones when trimming.
    const isSeed = (p) => this.seedProxies.includes(p);
    const keep = this.proxies.filter((p) => isSeed(p) || !this.isDead(p));
    ...
    // If curated hosts alone exceed the cap, keep them (they are the baseline)
    // and trim only the non-curated tail.
    let trimmed = next;
    if (next.length > MAX_POOL_SIZE) {
      const seeds = next.filter(isSeed);
      const extra = next.filter((p) => !isSeed(p));
      trimmed = seeds.concat(extra.slice(0, Math.max(0, MAX_POOL_SIZE - seeds.length)));
    }

    const added = trimmed.length - keep.length;
    const removed = this.proxies.length - keep.length;
    if (added > 0 || removed > 0) {
      this.proxies = trimmed;
      const fastest = good.find((r) => this.proxies.includes(r.proxyUrl));
      if (fastest) {
        this.preferredIndex = Math.max(0, this.proxies.indexOf(fastest.proxyUrl));
      } else if (this.preferredIndex >= this.proxies.length) {
        this.preferredIndex = 0;
      }
   ...
   *
   * @param {object} [options]
   * @param {boolean} [options.force] - Ignore the degradation check (still throttled)
   * @param {number} [options.waitMs] - Give up waiting after this long and let the
   *   pass finish in the background. A cold instance probing and then fetching a
   *   public list can take ~19s; a request must never wait that out.
   */
  async refreshIfDegraded({ force = false, waitMs = 0 } = {}) {
    ...
    if (waitMs > 0) {
      // Return null when the budget expires: the pass keeps running and its
      // results land in pool health for the next request.
      let timer;
      const expired = new Promise((resolve) => {
        timer = setTimeout(() => resolve(null), waitMs);
      });
      try {
        return await Promise.race([this.probeInFlight, expired]);
      } finally {
        clearTimeout(timer);
      }
    }
