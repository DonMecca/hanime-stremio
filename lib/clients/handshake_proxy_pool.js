...
class HandshakeProxyPool {
  ...
  getRotationOrder() {
    ...
    // Return [] when every proxy is cooling down, instead of falling back to the
    // full list. The old fallback meant a wholly-dead pool was re-tried in full
    // on every request, re-paying each connect timeout forever. Empty is also
    // what callers already handle: the handshake then reports that no proxy was
    // available rather than appearing to hang.
    return available;
  }
