...
const config = {
  server: {
    ...
    // Prefer explicit PUBLIC_URL (set on Netlify / local). Avoid relying on host-specific vars.
  ...
  api: {
    ...
    // Hanime retired search.htv-services.com (NXDOMAIN). Public catalog is now a
    // full-index GET dump; filtering/sorting/pagination happen client-side.
    searchUrl: 'https://guest.freeanimehentai.net/api/v11/search_hvs',
    searchIndexTtlMs: 30 * 60 * 1000, // 30 minutes
    // Authenticated/web player API (streams via signed handshake)
    authApiBase: 'https://auth.hanime.tv',
    csrfUrl: 'https://ct.hanime.tv/csrf-token',
    hlsBase: 'https://hanime.tv',
