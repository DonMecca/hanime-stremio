 ...
 * @param {Function} [options.shouldCache] - Override the default caching predicate
 * @returns {Promise} Cached result
 */
async function _cacheWrap(cache, key, method, ttl, options) {
  ...
  // Callers may tighten this with a predicate, e.g. to refuse empty result sets
  // that a transient upstream failure produced.
  const shouldCache = options && typeof options.shouldCache === 'function'
    ? options.shouldCache
    : _shouldCache;
 ...
 * @param {string} id - Cache key identifier
 * @param {Function} method - Async function to cache
 * @param {Object} [options] - Passed through to _cacheWrap
 * @returns {Promise} Cached result
 */
function cacheWrapStream(id, method, options) {
  return _cacheWrap(remoteCache, `${STREAM_KEY_PREFIX}:${id}`, method, STREAM_TTL, options);
}
